...
""",
    ...
    async def extract_dental_caries_code(self, scenario: str) -> dict:
        """Extract dental caries code(s), explanation, doubt, and include raw data."""
        raw_result = ""
        try:
            ...
            print(f"Dental caries extracted: Code={parsed_result.get('code')}, Exp={parsed_result.get('explanation')}, Doubt={parsed_result.get('doubt')}")
    ...
    async def activate_dental_caries(self, scenario: str) -> dict:
        """Activate the dental caries analysis and return simplified results."""
        try:
            # Await the extraction call which now returns the desired structure
            extraction_result = await self.extract_dental_caries_code(scenario)
            ...
            if not extraction_result.get("code") and not extraction_result.get("error"):
                 print("No dental caries code or error returned, but raw data might be present.")
    ...
    async def run_analysis(self, scenario: str) -> None:
        ...
        result = await self.activate_dental_caries(scenario) # Await the call
...
if __name__ == "__main__":
    ...
    async def main(): 
        ...
        await dental_caries_service.run_analysis(scenario) # Await the call
