...
class SymptomsAndDisordersOrthodonticsServices:
    """Class to analyze and extract Symptoms and Disorders Pertinent to Orthodontic Cases ICD-10 codes based on dental scenarios."""
    ...
    def _create_prompt_template(self) -> PromptTemplate:
        """Create the prompt template for analyzing Symptoms and Disorders Pertinent to Orthodontic Cases."""
        return PromptTemplate(
            template=f"""
You are an expert in dental medical coding, specifically for Symptoms and Disorders Pertinent to Orthodontic Cases.
Analyze the given scenario and identify the most appropriate ICD-10 code(s).

Symptoms and Disorders Pertinent to Orthodontic Cases ICD-10 codes include:
# Malocclusion (Major category)
...
K07.3: Anomalies of tooth position (Crowding, Diastema, Displacement, Rotation, Spacing, Transposition, Impacted/embedded with abnormal position)
K07.4: Malocclusion, unspecified
K07.5: Dentofacial functional abnormalities (Abnormal jaw closure, Abnormal swallowing, Mouth breathing, Tongue/lip/finger habits)
K07.8: Other specified dentofacial anomalies
K07.9: Dentofacial anomaly, unspecified
# Related Symptoms/Conditions
K00.6: Disturbances in tooth eruption (Delayed, Premature, Natal, Neonatal, Embedded, Impacted - if primary issue)
K01.0: Embedded teeth
K01.1: Impacted teeth
K08.89: Other specified disorders of teeth and supporting structures (Insufficient clinical crown length)
K13.1: Cheek and lip biting
K14.8: Other diseases of tongue (Tongue thrust)
G47.63: Sleep-related bruxism
M26.5: Dentofacial functional abnormalities (more specific codes under K07.5)
...
""",
    ...
    async def extract_symptoms_disorders_orthodontics_code(self, scenario: str) -> dict:
        """Extract Symptoms/Disorders Orthodontics code(s), explanation, doubt, and include raw data."""
        raw_result = ""
        try:
            print(f"Analyzing symptoms/disorders pertinent to orthodontics scenario: {scenario[:100]}...")
            ...
            print(f"Symptoms/Disorders Orthodontics extracted: Code={parsed_result.get('code')}, Exp={parsed_result.get('explanation')}, Doubt={parsed_result.get('doubt')}")
            ...
            print(f"Error in Symptoms/Disorders Orthodontics code extraction: {str(e)}")
    ...
    async def activate_symptoms_and_disorders_pertinent_to_orthodontic_cases(self, scenario: str) -> dict:
        """Activate the Symptoms/Disorders Orthodontics analysis and return simplified results."""
        try:
            # Await the extraction call which now returns the desired structure
            extraction_result = await self.extract_symptoms_disorders_orthodontics_code(scenario)
            ...
            if not extraction_result.get("code") and not extraction_result.get("error") and extraction_result.get("raw_data"):
                 print("No Symptoms/Disorders Orthodontics code or error returned, but raw data might be present.")

            return extraction_result # Return the dict directly
        except Exception as e:
            print(f"Error activating Symptoms/Disorders Orthodontics analysis: {str(e)}")
    ...
    async def run_analysis(self, scenario: str) -> None:
        ...
        result = await self.activate_symptoms_and_disorders_pertinent_to_orthodontic_cases(scenario) # Await the call
        print(f"\n=== SYMPTOMS AND DISORDERS PERTINENT TO ORTHODONTIC CASES ANALYSIS RESULT ===")
...
symptoms_and_disorders_orthodontics_service = SymptomsAndDisordersOrthodonticsServices()
# Example usage
if __name__ == "__main__":
    # Make main async
    async def main():
        scenario = input("Enter a scenario for Symptoms and Disorders Pertinent to Orthodontic Cases: ")
        await symptoms_and_disorders_orthodontics_service.run_analysis(scenario) # Await the call
