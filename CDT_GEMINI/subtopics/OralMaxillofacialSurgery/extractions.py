...
def create_extractions_extractor(temperature=0.0):
    """
    Create a LangChain-based extractions code extractor.
    """
...
def extract_extractions_code(scenario, temperature=0.0):
    """
    Extract extractions code(s) for a given scenario.
    """
    try:
        chain = create_extractions_extractor(temperature)
        result = chain.run(question=scenario)
        print(f"Extractions code result: {result}")
        return result.strip()
...
def activate_extractions(scenario):
    """
    Activate extractions analysis and return results.
    """
    try:
        return extract_extractions_code(scenario)
    except Exception as e:
