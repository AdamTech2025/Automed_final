...
const Signup = () => {
  ...
  useEffect(() => {
    ...
    const totalFields = 3; // name, email and password
    let filledFields = 0;
  ...
  const handleSubmit = (e) => {
    ...
    if (name && email && password) {
      // Show success modal
      setModalData({
        title: 'Welcome to Adam Tech!',
        message: 'Your account has been created! (Demo)',
        isSuccess: true
      });
      setShowModal(true);
      ...
      <div className="container mx-auto max-w-lg px-4 py-8 relative">
        <div className="card p-8 backdrop-blur-md bg-opacity-15 bg-white border border-white border-opacity-30 shadow-lg rounded-2xl">
          ...
          <p className="mt-4 text-center text-sm text-gray-300">
            Already have an account? <Link to="/" className="text-yellow-400 hover:underline">Login</Link>
          </p>
        </div>
      </div>
