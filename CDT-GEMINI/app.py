...
async def analyze_web(request: Request, scenario: str = Form(...)):
    ...
    try:
        ...
        template_data = {
            ...
            "show_questions_modal": has_questions,  # Flag to indicate if questions modal should be shown
            "processing_complete": True  # Flag to indicate processing is complete
            }
        
        # We'll always use index.html, but set a flag to show questions modal if needed
        return templates.TemplateResponse("index.html", template_data)
        
    except Exception as e:
        traceback.print_exc()  # Now this will work with the import
        return templates.TemplateResponse("index.html", {
            "request": request,
            "scenario": scenario,
